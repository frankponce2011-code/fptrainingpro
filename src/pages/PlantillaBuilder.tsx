...
function Step1Form({ plantilla, onBack, onNext }: {
  plantilla?: PlantillaRutina | null;
  onBack: () => void;
  ...
  function handleContinuar() {
    console.log('handleContinuar llamado', nombre);
    ...
    <div className="min-h-screen bg-gray-950 flex flex-col max-w-lg mx-auto">
      ...
      <main className="flex-1 px-4 py-6 pb-28 overflow-y-auto">
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-gray-400 mb-1.5">Nombre de la plantilla <span className="text-red-400">*</span></label>
            <input value={nombre} onChange={e => setNombre(e.target.value)} placeholder="ej. Rutina Pecho 4 dias"
...
function EjercicioPicker({ onSelect, onClose, title = 'Seleccionar ejercicio' }: {
  onSelect: (e: Ejercicio) => void; onClose: () => void; title?: string;
}) {
...
function EjercicioConfigModal({ ejercicio, existing, onSave, onClose }: {
  ...
  onSave: (cfg: Partial<PlantillaEjercicio> & { ejercicio_alternativo?: Ejercicio | null; ejercicio?: Ejercicio }) => void;
  onClose: () => void;
}) {
  ...
  if (showMainPicker) return (
    <EjercicioPicker title="Cambiar ejercicio"
      onSelect={e => { setMainEjercicio(e); setShowMainPicker(false); }}
      onClose={() => setShowMainPicker(false)} />
  );
  if (showAltPicker) return (
    <EjercicioPicker title="Ejercicio alternativo"
          ...
          {altEjercicio ? (
            <div className="flex items-center gap-3 bg-gray-800 rounded-xl p-3">
              {altEjercicio.imagen_url
                ...
                : <div className="w-9 h-9 rounded-lg bg-gray-700 flex items-center justify-center shrink-0"><Dumbbell size={14} className="text-gray-500" /></div>}
...
function KanbanBuilder({ plantilla, onBack, readOnly = false, onEditMode, onDuplicate }: {
  plantilla: PlantillaRutina; onBack: () => void;
  readOnly?: boolean; onEditMode?: () => void; onDuplicate?: () => void;
}) {
  ...
  async function loadDias() {
    const { data: diasData } = await supabase
      .from('plantilla_dias').select('*')
      .eq('plantilla_id', plantilla.id).order('numero_dia');
    ...
    const { data: ejData } = await supabase
      ...
      .in('dia_id', diasData.map(d => d.id)).order('orden');
  ...
  async function handleAddDay() {
    ...
    if (data) { setShowAddDay(false); setAddDayNum(''); setAddDayNombre(''); loadDias(); }
  }

  async function handleUpdateDiaNombre(diaId: string) {
    if (!tempDiaNombre.trim()) return;
    setUpdatingDay(true);
    const { error } = await supabase.from('plantilla_dias')
      .update({ nombre_dia: tempDiaNombre.trim() }).eq('id', diaId);
    if (!error) {
  ...
  async function handleSaveEjercicio(cfg: Partial<PlantillaEjercicio> & { ejercicio_alternativo?: Ejercicio | null; ejercicio?: Ejercicio }) {
    if (!selectedDia) return;
    const diaObj = dias.find(d => d.id === selectedDia)!;
    const payload = {
      dia_id: selectedDia,
      ejercicio_id: cfg.ejercicio_id ?? pendingEjercicio!.id,
  ...
  if (detailEj) return <EjercicioDetail ejercicioConfig={detailEj} onBack={() => setDetailEj(null)} />;
  ...
  return (
    <>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-t-2xl w-full p-5 pb-8">
            ...
            <div className="space-y-3 mb-5">
              <div>
                ...
                <input type="number" min={1} max={7} value={addDayNum} onChange={e => setAddDayNum(e.target.value)} placeholder="ej. 1"
                  className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-yellow-400" />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-400 block mb-1.5">Nombre del dia</label>
                <input type="text" value={addDayNombre} onChange={e => setAddDayNombre(e.target.value)} placeholder="ej. Pecho y Triceps"
                  className="w-full bg-gray-800 border border-gray-700 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-yellow-400" />
              </div>
            </div>
      ...
      {pendingEjercicio && (
        <EjercicioConfigModal ejercicio={pendingEjercicio} existing={editingPej}
        ...
        <header className="bg-gray-900 border-b border-gray-800 px-4 py-4 sticky top-0 z-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <button onClick={onBack} className="text-gray-400 hover:text-white transition-colors shrink-0"><ArrowLeft size={20} /></button>
                    ...
                    <div key={dia.id} ref={el => (cardRefs.current[idx] = el)}
                          ...
                          {editingDiaId === dia.id ? (
                            <div className="flex items-center gap-2 mt-1 pr-2">
                              <input type="text" value={tempDiaNombre} onChange={e => setTempDiaNombre(e.target.value)}
                                ...
                                autoFocus disabled={updatingDay}
                            ...
                            <div className="flex items-center gap-1.5 group cursor-pointer mt-0.5"
                              onClick={e => { if (readOnly) return; e.stopPropagation(); setEditingDiaId(dia.id); setTempDiaNombre(dia.nombre_dia); }}>
                              <p className="text-sm font-bold text-white truncate">{dia.nombre_dia}</p>
                              {!readOnly && <Edit3 size={11} className="text-gray-500 hover:text-yellow-400 transition-colors opacity-60 group-hover:opacity-100 shrink-0" />}
                            </div>
                          )}
                        </div>
                      ...
                      <div className={`flex-1 overflow-y-auto rounded-b-2xl p-3 space-y-2 ${isActive ? 'bg-gray-800/60' : 'bg-gray-800/30'}`}
                        ...
                        {grouped.map((item, idx) => {
                              ...
                              <div key={idx} className={`rounded-xl border ${borderClass} overflow-hidden`}>
                                {item.map(pej => (
                                  <div key={pej.id} className="flex items-stretch">
                                    ...
                                    <ExerciseCard pej={pej} grp={grp} isActive={isActive} readOnly={readOnly}
                          ...
                          return (
                            <ExerciseCard key={item.id} pej={item} isActive={isActive} readOnly={readOnly}
...
function ExerciseCard({ pej, grp, isActive, readOnly = false, onDetail, onEdit, onDelete }: {
  pej: PlantillaEjercicio; grp?: string; isActive: boolean; readOnly?: boolean;
  onDetail: () => void; onEdit: () => void; onDelete: () => void;
}) {
  const tipo = pej.tipo as TipoSerie;
  return (
    <div className="flex-1 bg-gray-900/80 p-3 overflow-hidden">
      <div className="flex items-center gap-2 min-w-0">
        <ExerciseImg url={pej.ejercicio?.imagen_url} alt=""
          className="w-9 h-9 rounded-lg object-cover"
          containerClassName="w-9 h-9 rounded-lg bg-gray-700 flex items-center justify-center shrink-0"
          fallbackSize={12} />
        <div className="flex-1 min-w-0 overflow-hidden cursor-pointer" onClick={onDetail}>
        ...
        {isActive && !readOnly && (
          <div className="flex items-center gap-0.5 shrink-0 ml-1">
            <button onClick={e => { e.stopPropagation(); onEdit(); }} className="p-1 text-gray-600 hover:text-yellow-400 rounded transition-colors">
              <Edit3 size={12} />
            </button>
            <button onClick={e => { e.stopPropagation(); onDelete(); }} className="p-1 text-gray-600 hover:text-red-400 rounded transition-colors">
              <Trash2 size={12} />
            </button>
          </div>
        )}
...
export default function PlantillaBuilder({ plantilla, onBack, onSaved }: Props) {
  ...
  async function handleStep1(nombre: string, descripcion: string) {
    console.log('handleStep1 llamado', nombre, 'user:', user?.id);
    if (currentPlantilla) {
      const { data, error } = await supabase.from('plantillas_rutina')
        .update({ nombre, descripcion }).eq('id', currentPlantilla.id).select().single();
      console.log('update result:', data, error);
      if (data) { setCurrentPlantilla(data); onSaved(data); }
    } else {
      const { data, error } = await supabase.from('plantillas_rutina')
        .insert({ nombre, descripcion, creado_por: user?.id }).select().single();
  ...
  async function handleDuplicate() {
    ...
    setDuplicating(true);
    const { data: newPlantilla } = await supabase.from('plantillas_rutina')
      .insert({ nombre: `${currentPlantilla.nombre} (copia)`, descripcion: currentPlantilla.descripcion, creado_por: user.id })
      .select().single();
    if (!newPlantilla) { setDuplicating(false); return; }

    const { data: diasData } = await supabase.from('plantilla_dias')
      .select('*, plantilla_ejercicios(*)')
      .eq('plantilla_id', currentPlantilla.id).order('numero_dia', { ascending: true });

    if (diasData) {
      for (const dia of diasData) {
        const { data: newDia } = await supabase.from('plantilla_dias')
        ...
        if (newDia && dia.plantilla_ejercicios?.length > 0) {
          await supabase.from('plantilla_ejercicios').insert(
            dia.plantilla_ejercicios.map((e: Omit<PlantillaEjercicio, 'ejercicio' | 'ejercicio_alternativo'>) => ({
              dia_id: newDia.id, ejercicio_id: e.ejercicio_id, orden: e.orden,
  ...
  return (
    <KanbanBuilder plantilla={currentPlantilla} onBack={onBack} readOnly={readOnly}
      onEditMode={() => setReadOnly(false)} onDuplicate={handleDuplicate} />
  );
}